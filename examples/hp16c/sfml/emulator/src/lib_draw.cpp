...
u16 FrameBuf[FRAMESIZE];
u16 DispWidth = WIDTH;
u16 DispHeight = HEIGHT;
...
sf::RenderWindow window(sf::VideoMode(320, 240), "HP16C emulator", sf::Style::Titlebar | sf::Style::Close);
...
void DispUpdate()
{
	...
	{
		// Set draw window (update area in the texture)
		auto* pixels = new sf::Uint8[(DispDirtyX2 - DispDirtyX1) * (DispDirtyY2 - DispDirtyY1) * 4]; // assuming 32-bit color depth
		for (int i = DispDirtyY1; i < DispDirtyY2; i++)
		{
			for (int j = DispDirtyX1; j < DispDirtyX2; j++)
			{
				// Calculate the position in the frame buffer
				u16* s0 = &FrameBuf[j + i*DispWidth];

				// Calculate the position in the pixel array
				int pixelPos = (j - DispDirtyX1 + (i - DispDirtyY1)*(DispDirtyX2 - DispDirtyX1)) * 4;

				pixels[pixelPos + 0] =  ((*s0 >> 8) & 0xf8) | ((*s0 >> 13) & 0x7); // R
				pixels[pixelPos + 1] = ((*s0 >> 3) & 0xfc) | ((*s0 >> 9)  & 0x3); // G
				pixels[pixelPos + 2] = ((*s0 << 3) & 0xf8) | ((*s0 >> 2)  & 0x7);      // B
				pixels[pixelPos + 3] = 255;                // A
			}
		}

		// Update the texture
		FrameTex.update(pixels, DispDirtyX2 - DispDirtyX1, DispDirtyY2 - DispDirtyY1, DispDirtyX1, DispDirtyY1);
		delete[] pixels;
